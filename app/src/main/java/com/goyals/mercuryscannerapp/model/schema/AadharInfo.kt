...
data class AadharInfo(@Json(name = "address") val address: String = "",
  ...
  @Nullable @Json(name = "location_name") val locationName: String? = "",
  ...
  @Nullable @Json(name = "year_of_birth") val yearOfBirth: String? = "") {
  val covidResultToShow: String = if (covidResult == 0) {
    "Negative"
  } else {
    "Positive"
  }
  val createdDateToShow =
    AppUtils.changeDateFormat(createdDate.substringBefore('T'))
}